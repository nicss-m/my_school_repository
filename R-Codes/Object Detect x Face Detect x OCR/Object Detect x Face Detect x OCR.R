...
needed.packages = c('image.darknet','magick','shiny','shinythemes','tesseract','shinybusy','shinyjs','image.libfacedetection')
...
ui <- fluidPage(
  ...
  titlePanel("Obj Detect x Face Detect x OCR"),
  sidebarLayout(
    sidebarPanel(
      conditionalPanel(condition = "input.tabselected==1 || input.tabselected==2 || input.tabselected==3 || input.tabselected==4", fileInput("file","Please Upload an Image", multiple = T),h6("Default max. file size is 5MB")),
      conditionalPanel(condition = "input.tabselected==4", selectInput("slcInput", "Select Language for Text Extraction", choices = tesseract_info()$available),actionButton("add_lang", "Add Language"),actionButton("save_text", "Save Text")),
      conditionalPanel(condition = "input.tabselected==1 || input.tabselected==2 || input.tabselected==3 || input.tabselected==4",h6("Powered by:"),tags$img(src='RStudio-Ball.png', height=50, width=50))
    ...
    mainPanel(
      tabsetPanel(type = "tab",
                  ...
                  tabPanel("Face Detection", value = 3, uiOutput("faceDetect")),
                  tabPanel("Text Extraction", value = 4, uiOutput("processingOutput")),
                  id = "tabselected"
    ...
    if(is.null(input$file)){}
      ...
      withProgress(message = 'Processing', value = 0,{
        ...
        if(grepl("\\.pdf$", file_name) || grepl("\\.ai$", file_name)){
          ...
          img = image_read(file_name)
          image_write(img, path ="image.png",format = 'png')
          img = image_read(file.path('image.png'))
        }else{
          img = image_read(file_name)
        }
        
        incProgress(1/n, detail = paste("Detecting faces..."))
        
        # detect face using image_detect_faces
        facedetect = image_detect_faces(img)
        incProgress(1/n, detail = paste("Plotting detected faces..."))
        
        # adding bounding box
        detected_faces = plot(facedetect,img,border="orange",lwd=5,col="white")
        
        incProgress(1/n, detail = paste("Writing image..."))
        
        # writing image
        image_write(detected_faces, path=paste0("www/",tools::file_path_sans_ext(input$file$name),"_face_detections.png"),format = 'png')
       
        tags$img(src = paste0(tools::file_path_sans_ext(input$file$name),"_face_detections.png"), height = 500, width = 590)
      })
    }
  )
